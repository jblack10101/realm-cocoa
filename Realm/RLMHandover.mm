...
@implementation RLMThreadHandover {
    ...
    std::unique_ptr<Realm::HandoverPackage> _package; // FIXME: Use `std::optional::optional` instead
...
- (instancetype)initWithRealm:(RLMRealm *)realm objects:(NSArray<id<RLMThreadConfined>> *)objects {
    if (self = [super init]) {
        ...
        _package = std::make_unique<Realm::HandoverPackage>(realm->_realm->package_for_handover(handoverables));
...
- (RLMThreadImport *)importOnCurrentThreadWithError:(NSError **)error {
    ...
    std::vector<AnyThreadConfined> handoverables = realm->_realm->accept_handover(std::move(*_package.release()));
