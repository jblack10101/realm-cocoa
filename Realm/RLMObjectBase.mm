...
+ (instancetype)rlm_objectWithHandoverData:(realm::AnyThreadConfined&)data
    ...
    RLMObjectSchema *schema = realm.schema[@(object.get_object_schema().name.c_str())];
    REALM_ASSERT(schema != nil);
    return RLMCreateObjectAccessor(realm, schema, object.row().get_index());
}
